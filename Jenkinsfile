  ...
  stages {
    stage('Build') {
      steps {
        ...
        configFileProvider([configFile(fileId: 'd547b34e-eab1-4a22-81d7-ddc2ba9ce198', targetLocation: '.env.production')])
        sh 'npm install && npm run build && tar -cf shoptrac-frontend.tar dist && bzip2 shoptrac-frontend.tar'
